...
bool Databaza::init() {
    std::string dbPath = "db/messages.db";
    int rc = sqlite3_open(dbPath.c_str(), &db_);
    if (rc != SQLITE_OK) {
        std::cerr << "Error to connect to DATABAZA: " << sqlite3_errmsg(db_) << std::endl;
        return false;
    }
...
bool Databaza::saveMessage(const std::string& source,
    ...
    if (rc != SQLITE_OK) {
        std::cerr << "Preparing request: " << sqlite3_errmsg(db_) << std::endl;
        return false;
    }
    ...
    if (rc != SQLITE_DONE) {
        std::cerr << "Insert data: " << sqlite3_errmsg(db_) << std::endl;
